...
const RecentListings = () => {
  ...
  const { data: listings, isLoading } = useQuery({
    queryKey: ['recent-listings-chugach'],
    queryFn: async () => {
  ...
  return (
    <section className="py-16 md:py-24 bg-background relative overflow-hidden">
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-0 left-1/4 w-96 h-96 bg-chugach-blue/5 rounded-full blur-3xl" />
        <div className="absolute bottom-1/4 right-1/4 w-80 h-80 bg-chugach-deep/5 rounded-full blur-3xl" />
      </div>
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        {isLoading ? (
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
              {displayedListings.map((listing, index) => (
                <Link
                  ...
                  style={{ animationDelay: `${Math.min(index * 30, 500)}ms`, animationFillMode: 'forwards' }}
                >
                  <div className="aspect-[4/3] bg-muted relative overflow-hidden">
                    {listing.images && listing.images.length > 0 ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-primary/10 to-chugach-deep/10">
                  ...
                  <div className="p-4">
                    <h3 className="font-display text-base font-semibold text-foreground mb-2 line-clamp-1 group-hover:text-primary transition-colors">{listing.title}</h3>
                    <div className="flex items-center gap-1.5 text-muted-foreground text-sm mb-2">
                      <MapPin className="w-3.5 h-3.5 flex-shrink-0" />
                      <span className="capitalize truncate">{listing.region.replace(/-/g, ' ')}</span>
                    </div>
                    <p className="text-muted-foreground text-xs line-clamp-2 mb-2">{listing.description}</p>
            ...
            {hasMore && (
              <div className="text-center mt-12">
                <Button variant="outline" size="lg" onClick={handleLoadMore} disabled={isLoadingMore} className="group min-w-[200px]">
                  {isLoadingMore ? (<><Loader2 className="w-4 h-4 mr-2 animate-spin" />Loading...</>) : (<>Show More<ChevronDown className="w-4 h-4 ml-2 group-hover:translate-y-0.5 transition-transform" /></>)}
                </Button>
              </div>
            )}
          </>
          ...
          <div className="text-center py-16 bg-card rounded-xl border border-border/30">
            ...
            <p className="text-muted-foreground mb-6 max-w-md mx-auto">Be the first to post a listing in the Chugach region</p>
            <Link to="/post-listing">
              <Button variant="chugach" size="lg">Post Your Listing — $10</Button>
            </Link>
          </div>
        )}
      </div>
